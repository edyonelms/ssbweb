...
<aside id="masterPanel" class="fixed inset-0 z-30 hidden" aria-hidden="true">
    ...
    <div id="masterPanelCard"
        ...
        <div class="px-5 py-3 border-b border-slate-200 flex items-center justify-between bg-white">
            <h3 id="masterPanelTitle" class="text-sm font-bold text-slate-800">Master Data</h3>
            <button type="button" onclick="MasterPanel.close()"
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            {{-- ────────────── UNIVERSITY: VIEW ────────────── --}}
            <div id="viewUniversity" class="master-mode hidden p-6 space-y-5">
                ...
                @if ($isAdmin)
                    <div class="flex items-center gap-2 pt-4 border-t border-slate-100">
                        <button type="button" id="viewUniversityEdit" class="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Edit</button>
                        <form id="viewUniversityDeleteForm" method="POST" action="" class="flex-1"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit" class="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 hover:bg-rose-50 hover:border-rose-200 text-rose-600 text-sm font-semibold transition">Delete</button>
                        </form>
                    </div>
                @endif
            </div>
            ...
            {{-- ────────────── COURSE: VIEW ────────────── --}}
            <div id="viewCourse" class="master-mode hidden p-6 space-y-5">
                ...
                @if ($isAdmin)
                    <div class="flex items-center gap-2 pt-4 border-t border-slate-100">
                        <button type="button" id="viewCourseEdit" class="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Edit</button>
                        <form id="viewCourseDeleteForm" method="POST" action="" class="flex-1"
            ...
            {{-- ────────────── FEE: VIEW ────────────── --}}
            <div id="viewFee" class="master-mode hidden p-6 space-y-5">
                <div class="pb-4 border-b border-slate-100">
                ...
                @if ($isAdmin)
                    <div class="flex items-center gap-2 pt-4 border-t border-slate-100">
                        <button type="button" id="viewFeeEdit" class="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Edit</button>
                        <form id="viewFeeDeleteForm" method="POST" action="" class="flex-1"
            ...
            @if ($isAdmin)
                {{-- ────────────── UNIVERSITY: CREATE ────────────── --}}
                <form id="formUniversityCreate" method="POST" action="{{ route('master.universities.store') }}" enctype="multipart/form-data" class="master-mode hidden p-6 space-y-4">
                    @csrf
                    @include('master._fields_university')
                    <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                        <button type="button" onclick="MasterPanel.close()" class="px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Cancel</button>
                ...
                {{-- ────────────── UNIVERSITY: EDIT ────────────── --}}
                <form id="formUniversityEdit" method="POST" action="" enctype="multipart/form-data" class="master-mode hidden p-6 space-y-4">
                    @csrf @method('PUT')
                    @include('master._fields_university')
                    <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                        <button type="button" onclick="MasterPanel.close()" class="px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Cancel</button>
                ...
                {{-- ────────────── COURSE: CREATE ────────────── --}}
                <form id="formCourseCreate" method="POST" action="{{ route('master.courses.store') }}" class="master-mode hidden p-6 space-y-4">
                    @csrf
                ...
                {{-- ────────────── COURSE: EDIT ────────────── --}}
                <form id="formCourseEdit" method="POST" action="" class="master-mode hidden p-6 space-y-4">
                    @csrf @method('PUT')
                    @include('master._fields_course', ['allUniversities' => $allUniversities])
                    <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                        <button type="button" onclick="MasterPanel.close()" class="px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Cancel</button>
                ...
                {{-- ────────────── FEE: CREATE ────────────── --}}
                <form id="formFeeCreate" method="POST" action="{{ route('master.fees.store') }}" class="master-mode fee-form hidden p-6 space-y-4">
                    @csrf
                ...
                {{-- ────────────── FEE: EDIT ────────────── --}}
                <form id="formFeeEdit" method="POST" action="" class="master-mode fee-form hidden p-6 space-y-4">
                    @csrf @method('PUT')
                    @include('master._fields_fee', ['allUniversities' => $allUniversities])
                    <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                        <button type="button" onclick="MasterPanel.close()" class="px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-semibold transition">Cancel</button>
