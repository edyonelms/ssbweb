    ...
    <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        ...
        <div>
            ...
            <select id="courseSelect" disabled
                    class="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-pink-300/60 focus:border-pink-300/60 outline-none transition text-sm disabled:bg-slate-50 disabled:text-slate-400">
                <option value="">Pick a university first</option>
            </select>
    ...
    (function () {
        ...
        function rebuildCourseDropdown() {
            ...
            if (!uniId) {
                ...
                courseSelect.firstChild.textContent = 'Pick a university first';
                courseHint.textContent = '—';
                return;
            }

            const matches = window.COURSES_DATA.filter(c => c.university_id === uniId);
            if (matches.length === 0) {
                courseSelect.disabled = true;
                courseSelect.firstChild.textContent = 'No courses for this university yet';
            ...
            courseHint.textContent = matches.length + ' course' + (matches.length === 1 ? '' : 's') + ' available.';
        }
        ...
        courseSelect.addEventListener('change', recalc);
